...
function load() {
    ...
    const account = storage.get("accountData") || {}
    const assignmentData = storage.get("assignmentData") || []
    ...
    const schema = {
        ...
        points: "number", // the points accumulated by the user
        assignments: "array", // the list of assignments
        assignmentData: "object", // the data related to assignments
    }
...
const options = {
    state: load,
    getters: {
        name: statef("fullName", getFirstName),
        accountData(state) {
            const keys = [
                "username",
                "fullName",
            ...
            return reduceState(state, keys)
        },
        assignmentData(state) {
            return state.assignmentData
        },
        points(state) {
            return sum(state.assignments.map((x) => x.score || 0))
        }
    },
    actions: {
        // any time the user leaves, we should sync the storage
        syncLocalStorage() {
            const storage = new ExpirationalStorage({
                name: "bkl",
            })
            storage.set("accountData", this.accountData)
            storage.set("assignmentData", this.assignmentData)
            // raw assignmentData (never mutated)
        },

        // accountData is largely static information
        // what really changes is the assignments
        // this is where points and statuses are stored
        async syncFirestore() {
            await updateChildDocuments(
                "users",
                this.username,
                "assignments",
                this.assignments,
