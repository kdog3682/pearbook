import {
    ExpirationalStorage,
} from "/home/kdog3682/2024-javascript/js-toolkit/LocalStorage.js"
// import {
    // createNamedDocument,
    // readDocument,
    // readDocuments,
...
const definition = () => {
    console.log('loading usePiniaFetchStudentData')
    ...
    async function getAccount(username, password, rememberMe) {
        console.log(accountDataStorageKey)
        const cached = storage.get(accountDataStorageKey)
        console.log("cached", cached)
        if (cached) {
            status.value = "success.loggedInViaCache"
            accountData.value = cached
            return true
        }
        if (arguments.length == 0) {
            console.log('could not find an account')
            return false
        }
        status.value = "attempt.loggingIn"
        const json = await loader(readDocument, "users", username.value)

        if (json) {
            if (password.value == json.password) {
                status.value = "success.loggedIn"
                accountData.value = json
                if (rememberMe.value) {
                    storage.set(accountDataStorageKey, json, {months: 2})
                }
                return true
            } else {
    ...
    async function getAssignments() {
        ...
        if (!accountData.value) {
            // u are not allowed to retrieve assignment data
            // unless u r logged in
            status.value = 'error:userNotLoggedIn'
        }
        const cached = storage.get(assignmentDataStorageKey)
        if (exists(cached)) {
            assignmentData.value = cached
            console.log('got assignmentData from cache')
            console.log(assignmentData.value)
            console.log(cached)
            return true
        }
        const json = await loader(
            readDocuments, 'users', accountData.value.username, 'assignments'
        )
        if (exists(json)) {
            assignmentData.value = json 
            storage.set(assignmentDataStorageKey, json)
            return true
        } else {
            const j = [1,2,3]
            assignmentData.value = j
            console.log('getting fallback assingmentData')
            storage.set(assignmentDataStorageKey, j, 1)
            return true
        }
    }

    async function syncAccountLocally() {
        const expiry = null
        storage.set(accountDataStorageKey, accountData.value, expiry)
    }

    async function syncAssignmentsLocally() {
        const expiry = null
        storage.set(assignmentDataStorageKey, assignmentData.value, expiry)
    }
    ...
    async function createAccount(username, payload) {
        const user = await loader(readDocument, 'users', username)
        if (user) {
            status.value = 'error:userAlreadyExists'
            return false
        } else {
            await createNamedDocument('users', username, payload)
            return true
        }
    }
