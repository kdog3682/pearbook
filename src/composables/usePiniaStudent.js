...
const usePostFirebase = () => {
    const {
        accountData, // reactive
        assignmentItems, // ref
        assignmentPosts, //shallowRef
        username,
    } = usePiniaStudent()

    const postAssignmentData = async (id) => {
        const payload = assignmentItems.value.find((x) =>
            x.id == id
        )
    ...
    // we do not post assignmentPosts as they are immutable
    return {
        postAccountData,
        postAssignmentData,
    }
...
function set(data, doc) {
    if (doc) {
        data.value = doc
        return doc
    }
}
const piniaStudentDef = () => {

    const storage = new Storage({ name: "bkl" })
    const baseAccountData = storage.get('accountData') || {}
    const baseAssignmentData = storage.get('assignmentItems') || []
    const baseAssignmentPosts = storage.get('assignmentPosts') || {}
    // loading existing data on piniaStudentDef startup
    // this will always happen and happen once
    ...
    const accountData = reactive(baseAccountData)
    const assignmentItems = ref(baseAssignmentData)
    const assignmentPosts = shallowRef(baseAssignmentPosts)

    const setRememberMe = (rememberMe) => {
        memory.value = toValue(rememberMe)
    }
    const ensureAssignmentPosts = async () => {
        if (empty(assignmentPosts.value)) {
            await fetchAssignmentPosts()
        }
    }

    const ensureAssignments = async () => {
        if (empty(assignmentItems.value)) {
            await hybridFetchAssignmentData()
        }
    }
    ...
    const fetchAssignmentData = async () => {
        ...
        if (doc) {
            assignmentItems.value = doc
            return doc
        }
    }

    const hybridFetchAssignmentPosts = async () => {
        const posts = assignmentPosts.value
        for (const arg of assignmentItems.value) {
            if (!posts.hasOwnProperty(arg.assignmentId)) {
        ...
        storage.set("assignmentPosts", posts, {years: 1})
        return posts
    }

    const fetchAccountData = async (overrideUser) => {
        const doc = await readDocument("users", overrideUser || username.value)
        if (doc) {
            storage.set('accountData', doc)
            Object.assign(accountData, doc)
            return accountData
        }
    }
...
export const usePiniaStudent = defineStore(
    "usePiniaStudent",
    piniaStudentDef,
)
