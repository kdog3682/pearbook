...
function gradeLevelFromAge(age) {
    switch(age) {
        ...
        case 19: return 'COLLEGE-FRESHMAN'
        case 20: return 'COLLEGE-SOPHOMORE'
        case 21: return 'COLLEGE-JUNIOR'
        case 22: return 'COLLEGE-SENIOR'
        default: return `Grade ${age - 5}`
    }
}
...
function createUser(name) {
    const ref = 
    const getUsername = (o) => {
        return o.username || o.fullName.toLowerCase().replace(/ /g, '')
    }
    const o = userAccounts.find((x) => x.active && getUsername(x) === name)
    const username = getUsername(o)
    const password = o.accountType == 'NPC' ? 'icecream' : null
    const age = calculateAge(o.estimatedBirthYear)
    const gradeLevel = gradeLevelFromAge()

    const assignments = createStartingAssignments()

    const groupMembers = userAccounts.reduce((acc, user, i) => {
        if (user.studentGroup) {
            acc.add(user.studentGroup, getUsername(user))
        }
        return acc
    }, new Storage)

    const cohortMembers = userAccounts.reduce((acc, user, i) => {
        if (user.cohort) {
            acc.add(user.cohort, getUsername(user))
        }
        return acc
    }, new Storage)
    const groupMembers = 
