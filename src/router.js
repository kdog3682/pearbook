...
function get(name, useFallback = true) {
    ...
    if (useFallback) {
        return {
            template: `<p>Fallback Component: ${name}</p>`,
        }
    }
}

function createRouteFromString(s) {
    let [p, name] = mget3(s, / +as +(\S+)/)
    // console.log({p, name})
    let path = p
    if (!name) {
        name = s
...
function create(route) {
    if (isString(route)) {
        route = createRouteFromString(route)
        // console.log(route)
    }
