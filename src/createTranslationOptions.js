...
internal links
`
function parseI18n(s) {
    const items = split(s, /\n\n+/).map((x) => split(x, /\n/))
    const en = {}
    const chi = {}

    items.forEach(([key, english, chinese]) => {
        dictSetter4(en, key, english)
        dictSetter4(chi, key, chinese)
    })
    return {
        'english': en, 
        'chinese': chi,
...
function dictSetter4(base, key, value, mergeStrategy) {
    let ref = base
    let keys = key.split('.')
