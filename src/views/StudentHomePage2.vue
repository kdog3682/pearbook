    ...
    div
        ...
        ul
            li(v-for = 'link in computedLinks')
                router-link(:to = 'computeLinkAddress(link)') {{$t(link.label)}}

        .router-container
            router-view

        .assignments
            assignment-snippet(v-for = "id in assignments" :assignmentId = 'id')
...
const links = shallowRef([
        {       name: 'student-details', label: 'details' },
        {       name: 'student-timeline', label: 'timeline', condition: {completedAssignments: '>4' }},
])
// lorem 
const computedLinks = computed(() => {
    return links.value.filter((link, i) => {
        if (link.condition) {
            if (isFunction(link.condition)) {
                 return link.condition(account.value)
...
await pinFetch.getAccount()
await pinFetch.getAssignments()
const account = pinFetch.accountData
const assignments = pinFetch.assignmentData
