...
import { inAndOut } from './lop.js'

function getAttrs(s) {
    const items = split(s, /: +/)
    if (items.length > 1) {
        return [items[0], toArgument(items[1])]
    }
}
...
function parse(el) {
    ...
    if (ind > 0 && el.parent?.text?.includes('/')) {
        const prefix = depluralize(split(el.parent.text, '/')[0])
        name = `${prefix}-${name}`
        component = name
    }
...
function onLeaf(el) {
    if (el.ind === 0) {
        ...
        const attrs = getAttrs(el.text)
        if (attrs) {
            el.parent.assign('attrs', ...attrs)
            el.remove()
        } else {
            el.assign(parse(el))
        }
...
function onBranchExit(el) {
    // console.log(el.text)
    // return
    const home = el.get('attrs', 'home')
    let payload = {}
    let s = el.text || ''
    let path = s
    let component = null
    let props = path.includes(':')
    const parts = split(s, '/')
    let name = ''
    let meta = {}

    if (s.includes('username')) {
        const key = depluralize(parts[0])
        meta['for'] = key
    }
